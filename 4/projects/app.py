...
def export_data():
    ...
    try:
        ...
        if export_format == 'csv':
            filename = f'香连止痢丸检测数据_{timestamp}.csv'
            ...
            with open(filepath, 'w', encoding='utf-8-sig') as f:
                ...
                # 写入表头
                f.write("序号,直径(mm),X坐标(px),Y坐标(px),半径(px),面积(px),圆形度\n")
                
                # 写入数据
                for i, ball in enumerate(balls, 1):
                    f.write(f"{i},{ball.get('diameter', 0):.3f},{ball.get('x', 0):.1f},{ball.get('y', 0):.1f},{ball.get('radius', 0):.1f},{ball.get('area', 0):.1f},{ball.get('circularity', 0):.3f}\n")
