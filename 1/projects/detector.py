...
class BallDetector:
    ...
    def __init__(self):
        ...
        # 标定框实际尺寸 (mm)
        self.calibration_width = 20   # 2cm
        self.calibration_height = 20  # 2cm
    ...
    def detect_black_frame(self, image):
        ...
        # 步骤2: 二值化 - 检测深色线条
        # THRESH_BINARY_INV: 深色区域变为白色
        line_threshold = 80
        _, binary = cv2.threshold(gray, line_threshold, 255, cv2.THRESH_BINARY_INV)
        ...
        # 计算中心点和百分比参数
        center_x = (x + w / 2) / width * 100 - 50  # 转换为偏移百分比
        center_y = (y + h / 2) / height * 100 - 50
        frame_width_pct = w / width * 100
        frame_height_pct = h / height * 100
        
        return {
            'found': True,
            'offsetX': round(center_x, 1),
            'offsetY': round(center_y, 1),
            'frameWidth': round(frame_width_pct - 2),  # 缩小2%确保框在内部
            'frameHeight': round(frame_height_pct - 2),
            'pixelWidth': w,
            'pixelHeight': h
        }
