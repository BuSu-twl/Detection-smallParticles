    ...
    def _detect_actual_black_frame(self, image):
        ...
        for contour in contours:
            ...
            if len(approx) == 4:
                ...
                # 验证是否为有效的黑框（接近正方形，允许透视变形）
                aspect_ratio = max(w, h) / min(w, h)
                
                # 计算轮廓的完整性（四个角是否清晰）
                # 通过计算角点到轮廓的平均距离来评估
                contour_box_area = w * h
                fill_ratio = area / contour_box_area if contour_box_area > 0 else 0
                
                # 计算得分：考虑面积占比、宽高比、填充率
                area_ratio = area / (width * height)
                aspect_score = 1.0 / aspect_ratio if aspect_ratio < 2.0 else 0.3
                fill_score = fill_ratio if fill_ratio > 0.5 else fill_ratio * 0.5
                
                score = area_ratio * aspect_score * fill_score * 100
        ...
        if valid_quads:
            ...
            # 验证得分是否足够高
            if best['score'] > 1.0:  # 得分阈值
                # 可选：检查是否有其他候选，如果有多个候选且得分接近，使用面积居中的那个
                if len(valid_quads) > 1 and valid_quads[1]['score'] > best['score'] * 0.8:
                    # 多个候选，选择面积适中的
                    areas = [q['area'] for q in valid_quads[:3]]
                    median_area = sorted(areas)[len(areas)//2]
                    best = next(q for q in valid_quads if q['area'] == median_area)
                ...
                return {
                    ...
                    'w': int(best['w']),
                    'h': int(best['h']),
                    'corners': best['corners'].tolist()
                }
        
        # 方法2：如果没找到四边形，使用优化的梯度扫描
